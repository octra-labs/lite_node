...
let default_content_type =
  "application/vnd.octra.balance-cell+ciphertext"

let base_key path_key suffix =
  "balance_cell:" ^ path_key ^ ":" ^ suffix

let ciphertext_commitment_key path_key =
  base_key path_key "ciphertext_commitment"

let amount_commitment_key path_key =
  base_key path_key "amount_commitment"
...
let yojson_of_t (cell : t) =
  `Assoc [
    ...
    "amount_commitment",
    begin
      match cell.amount_commitment with
      | Some value -> `String value
      | None -> `Null
    end;
    "proof_kind",
...
let yojson_of_put_payload (payload : put_payload) =
  `Assoc [
    ...
    "amount_commitment", `String payload.amount_commitment;
    "proof_kind",
    begin
...
let ensure_descriptor_compatible (descriptor : Circle_state_descriptor.t) =
  if descriptor.state_class <> Circle_state_descriptor.Balance_cell then
    Error "balance cell requires balance_cell descriptor class"
  else if not (compatible_hfhe_profile descriptor.hfhe_profile) then
    Error "balance cell requires balance_cell_v1 or commit_bound_v1 hfhe profile"
  else
    Ok descriptor
...
let normalize_cell
  ...
  match
    ...
    normalize_commitment "amount_commitment" amount_commitment,
    normalize_optional_proof_kind proof_kind,
    normalize_optional_hex64 "proof_receipt_hash" proof_receipt_hash
  with
  | Ok ciphertext_commitment, Ok amount_commitment, Ok proof_kind, Ok proof_receipt_hash ->
    begin
    ...
    begin
      match
        Circle_state_descriptor.resolve_put_payload
          {
            state_ref;
            state_class = Some Circle_state_descriptor.Balance_cell;
            codec = payload.codec;
            ...
            hfhe_profile =
              Some
                (Option.value
                   ~default:Circle_state_descriptor.Balance_cell_v1
...
let write_snapshot storage_tbl path_key (cell : t) =
  ...
  set_or_clear
    storage_tbl
    (amount_commitment_key path_key)
    cell.amount_commitment;
  set_or_clear
    storage_tbl
...
let materialized (cell : t) =
  ...
  || Option.is_some cell.amount_commitment
...
let validate_runtime_key raw_key value =
  ...
  | [raw_prefix; path_key; suffix] when raw_prefix = "balance_cell" ->
    if String.length path_key <> 64 || not (String.for_all Circles.is_hex_char path_key) then
      Error ("circle_runtime_invalid_balance_cell_key", raw_key, "path_key")
    else if not (runtime_suffix_allowed suffix) then
      Error ("circle_runtime_invalid_balance_cell_key", raw_key, suffix)
    else
      begin
        match suffix with
        | "ciphertext_commitment"
        | "amount_commitment" ->
          begin
            ...
            | Ok _ -> Ok ()
            | Error _ -> Error ("circle_runtime_invalid_balance_cell_value", raw_key, suffix)
          end
        | "proof_kind" ->
          begin
            match Circle_hfhe_proof.of_string (String.trim value) with
  ...
  | _ ->
    Error ("circle_runtime_invalid_balance_cell_key", raw_key, "shape")
  ...
  | `Assoc fields ->
    begin
      match
        ...
        Circle_private_common.read_required_string_field "amount_commitment" fields,
        Circle_private_common.read_optional_enum_field
          "proof_kind"
          fields
      ...
      | Ok state_ref, Ok content_type, Ok encoding, Ok key_id, Ok plaintext_hash,
        ...
        Ok ciphertext_commitment, Ok amount_commitment, Ok proof_kind,
        Ok proof_receipt_hash ->
        begin
      ...
      | Error e, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _
      | _, Error e, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _
      | _, _, Error e, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _
      | _, _, _, Error e, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _
      | _, _, _, _, Error e, _, _, _, _, _, _, _, _, _, _, _, _, _, _
      | _, _, _, _, _, Error e, _, _, _, _, _, _, _, _, _, _, _, _, _
      | _, _, _, _, _, _, Error e, _, _, _, _, _, _, _, _, _, _, _, _
      | _, _, _, _, _, _, _, Error e, _, _, _, _, _, _, _, _, _, _, _
      | _, _, _, _, _, _, _, _, Error e, _, _, _, _, _, _, _, _, _, _
      | _, _, _, _, _, _, _, _, _, Error e, _, _, _, _, _, _, _, _, _
      | _, _, _, _, _, _, _, _, _, _, Error e, _, _, _, _, _, _, _, _
      | _, _, _, _, _, _, _, _, _, _, _, Error e, _, _, _, _, _, _, _
      | _, _, _, _, _, _, _, _, _, _, _, _, Error e, _, _, _, _, _, _
      | _, _, _, _, _, _, _, _, _, _, _, _, _, Error e, _, _, _, _, _
      | _, _, _, _, _, _, _, _, _, _, _, _, _, _, Error e, _, _, _, _
      | _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, Error e, _, _, _
      | _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, Error e, _, _
      | _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, Error e, _
      | _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, Error e ->
        Error e
    end
  | _ ->
    Error "balance cell payload must be an object"