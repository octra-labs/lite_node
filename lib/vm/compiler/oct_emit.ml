...
let rec typ_of_expr env = function
  ...
  | EEpoch ->
    if env.checked then
      Option.value (Option.map (fun (_, _, typ) -> typ) (find_local env "epoch"))
        ~default:TInt
    else TInt
  | EEpochTime ->
    if env.checked then
      Option.value
        (Option.map (fun (_, _, typ) -> typ) (find_local env "epoch_time"))
        ~default:TInt
    else TInt
  | EValue ->
    if env.checked then
      Option.value (Option.map (fun (_, _, typ) -> typ) (find_local env "value"))
        ~default:TInt
    else TInt
  | EBalance _ -> TInt
  | ETreeHash | ENodeId | ETxHash -> TString
  | EVar "epoch" when not env.checked || Option.is_none (find_local env "epoch") ->
    typ_of_expr env EEpoch
  | EVar "epoch_time"
      when not env.checked || Option.is_none (find_local env "epoch_time") ->
    typ_of_expr env EEpochTime
  | EVar "value" when not env.checked || Option.is_none (find_local env "value") ->
    typ_of_expr env EValue
  | EVar name ->
  ...
  | ELet (name, _, declared, _, body) ->
    ...
    let result = typ_of_expr env body in
    env.locals <- saved;
    result
  | ESplit (_, (left, _, left_typ), (right, _, right_typ), body) ->
    ...
    env.locals <- saved;
    result
  | EOrbit (_, _, _, (_, _, declared), _) -> declared
...
and gen_expr env expr =
  ...
  | EEpoch ->
    if env.checked then
      match find_local env "epoch" with
      | Some (_, reg, _) -> reg
      | None ->
        let r = alloc_reg env in
        emit env (Contract_vm.EPOCH r);
        r
    else
      let r = alloc_reg env in
      emit env (Contract_vm.EPOCH r);
  ...
  | EEpochTime ->
    if env.checked then
      match find_local env "epoch_time" with
      | Some (_, reg, _) -> reg
      | None ->
        if env.declaration <> ProgramDecl then
          ...
          let r = alloc_reg env in
          emit env (Contract_vm.EPOCH_TIME r);
          r
    else if env.declaration <> ProgramDecl then
      gerr env.line "epoch_time is available only in Program"
    else
      let r = alloc_reg env in
      emit env (Contract_vm.EPOCH_TIME r);
  ...
  | EValue ->
    if env.checked then
      match find_local env "value" with
      | Some (_, reg, _) -> reg
      | None ->
        let r = alloc_reg env in
        emit env (Contract_vm.VALUE r);
        r
    else
      let r = alloc_reg env in
      emit env (Contract_vm.VALUE r);
...
let emit_update_value env op typ left_r right_r =
  let result_r = alloc_reg env in
  begin
    match op with
