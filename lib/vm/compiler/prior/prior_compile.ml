...
type compile_result = Oct_compile.compile_result = {
  bytecode : string;
  abi_json : string;
  instructions : int;
  error : string option;
...
let verification_json ast =
  Prior_verify.verify_ast ast
  |> Prior_verify.json_of_report
...
let require_ast_shape ast =
  let imports = ast.Prior_lang.imports in
  let interfaces = ast.Prior_lang.interfaces in
  if List.length imports > Program_limits.max_imports then
    raise (Compile_limit "Program import count exceeds compiler limit")
  else if
    total_length imports (fun imp -> List.length imp.Prior_lang.imp_names)
  ...
  then
    ...
    total_length interfaces (fun iface -> List.length iface.Prior_lang.if_methods)
...
let interface_index ast =
  let index = Hashtbl.create (List.length ast.Prior_lang.interfaces) in
  List.iter
    (fun iface ->
       let name = iface.Prior_lang.if_name in
       if Hashtbl.mem index name then
         raise (Compile_limit ("Program interface is duplicated: " ^ name))
       else
         Hashtbl.add index name iface)
    ast.Prior_lang.interfaces;
  index

let flow_kind = function
  | Prior_lang.TInt -> Program_type_flow.Int
  | Prior_lang.TBool -> Program_type_flow.Bool
  | Prior_lang.TString -> Program_type_flow.String
  | Prior_lang.TAddress -> Program_type_flow.Addr
  | Prior_lang.TBytes -> Program_type_flow.Bytes
  | Prior_lang.TBytes32 -> Program_type_flow.Bytes32
  | Prior_lang.TU64 -> Program_type_flow.U64
  | Prior_lang.TU128 -> Program_type_flow.U128
  | Prior_lang.TU256 -> Program_type_flow.U256
  | Prior_lang.TCipher -> Program_type_flow.Cipher
  | Prior_lang.TPubKey -> Program_type_flow.PubKey
  | Prior_lang.TMap _
  | Prior_lang.TList _
  | Prior_lang.TStruct _
  | Prior_lang.TEnum _
  | Prior_lang.TOption _
  | Prior_lang.TTuple _
  | Prior_lang.TVoid -> Program_type_flow.Unknown

let param_facts params =
  params
  |> List.mapi (fun i param -> (1001 + i, flow_kind param.Prior_lang.p_typ))

let facts_of_ast ast code =
  let labels = Hashtbl.create (List.length ast.Prior_lang.funcs + 1) in
  Array.iteri
    (fun pc op ->
      match op with
    ...
    code;
  let root =
    match ast.Prior_lang.ctor with
    | Some ctor -> param_facts ctor.Prior_lang.fn_params
    | None -> []
  in
  let storage =
    ast.Prior_lang.state
    |> List.filter_map (fun field ->
      let kind = flow_kind field.Prior_lang.sf_typ in
      match kind with
  ...
  in
  let entries =
    ast.Prior_lang.funcs
    |> List.mapi (fun i fn ->
      match Hashtbl.find_opt labels (200 + i * 100) with
      | Some target ->
        Some { Program_type_flow.target; mem = param_facts fn.Prior_lang.fn_params; effects = [] }
      | None -> None)
    |> List.filter_map (fun value -> value)
  in
  let functions = Hashtbl.create (List.length ast.Prior_lang.funcs) in
  List.iteri
        ...
        (match
         ...
         | Some target, Some owner, Some fn ->
           found := {
             ...
             pc;
             target;
             kind = flow_kind fn.Prior_lang.fn_ret;
           } :: !found
         | _ -> ())
      | _ -> ())
      code;
...
let certificate_json ~declaration ~source_mode ~source_material ~bytecode ~verification_json =
  ...
  `Assoc [
    ...
    "verification_schema", `String Prior_verify.schema;
...
let compile_ast_ready ~source_mode ~source_material ast =
  let declaration = Prior_lang.declaration_to_string ast.Prior_lang.declaration in
  let program = ast.Prior_lang.declaration = Prior_lang.ProgramDecl in
  if program
     && List.length ast.Prior_lang.funcs > Program_limits.max_functions then
    error_result "Program function limit exceeded"
  else
    let code = Prior_gen.generate ast in
    if program && Array.length code > Program_limits.max_instructions then
      error_result "Program instruction limit exceeded"
    else
      let abi_json = Prior_gen.to_abi ast |> abi_json declaration in
      let bytecode = Bytecode.encode ~active:false code in
...
let compile_ast_first ~source_mode ~source_material ast =
  compile_ast_ready
    ~source_mode
    ~source_material
    { ast with Prior_lang.interfaces = first_interfaces ast.Prior_lang.interfaces }
...
let imported_interfaces load_import ast =
  let bindings = Hashtbl.create (List.length ast.Prior_lang.imports) in
  let add_import acc (imp : Prior_lang.import_decl) name =
      ...
      raise
        (Compile_limit
           (Printf.sprintf
              ...
              name
              imp.Prior_lang.imp_path))
    | Some iface ->
      begin
        match Hashtbl.find_opt bindings name with
        | None ->
          Hashtbl.add bindings name imp.Prior_lang.imp_path;
          iface :: acc
        | Some path when String.equal path imp.Prior_lang.imp_path -> acc
        | Some _ ->
          raise
            (Compile_limit
  ...
  in
  List.fold_left
    (fun acc (imp : Prior_lang.import_decl) ->
       List.fold_left
         (fun acc name -> add_import acc imp name)
         acc
         imp.Prior_lang.imp_names)
    []
    ast.Prior_lang.imports
  |> List.rev

let imported_interfaces_first load_import ast =
  let select (seen, selected) (imp : Prior_lang.import_decl) =
    let requested =
      List.fold_left
        ...
        imp.Prior_lang.imp_names
    in
    (load_import imp).Prior_lang.interfaces
    |> List.fold_left
         (fun (seen, selected) iface ->
           let name = iface.Prior_lang.if_name in
...
let merge_interfaces ast interfaces =
  { ast with Prior_lang.interfaces = interfaces @ ast.Prior_lang.interfaces }
...
let compile source =
  try
    let ast = Prior_parse.parse source in
    compile_ast ~source_mode:"single" ~source_material:source ast
  with
  | Prior_lex.LexError (msg, line, _col) ->
    error_result (Printf.sprintf "line %d: %s" line msg)
  | Prior_parse.ParseError (msg, line) ->
    error_result (Printf.sprintf "line %d: %s" line msg)
  | Prior_gen.GenError (msg, line) ->
    error_result (Printf.sprintf "line %d: %s" line msg)
...
let compile_multi_with
  ...
  try
    ...
    in
    let load_import imp =
      let path = imp.Prior_lang.imp_path in
    ...
    let ast = Prior_parse.parse main_source in
    check_ast ast;
    if program_only && ast.Prior_lang.declaration <> Prior_lang.ProgramDecl then
...
let emit_program_with_facts result facts =
  ...
  | Some _ -> result
  | None ->
    (match Bytecode.decode ~active:false result.bytecode with
...
let compile_program source =
  try
    let ast = Prior_parse.parse source in
    if ast.Prior_lang.declaration <> Prior_lang.ProgramDecl then
      error_result "Program declaration required"
    else
      emit_program
        (compile_ast ~source_mode:"single" ~source_material:source ast)
