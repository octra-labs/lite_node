...
let epoch_hash_i64 ~epoch_id items =
...
let epoch_hash ~epoch_id items =
  epoch_hash_i64 ~epoch_id:(Int64.of_int epoch_id) items
...
let next_root_i64 ~prev ~epoch_id items =
  let epoch_hash = epoch_hash_i64 ~epoch_id items in
  let epoch_index_root = root_hash ~prev ~epoch_hash in
  epoch_hash, epoch_index_root

let next_root ~prev ~epoch_id items =
  next_root_i64 ~prev ~epoch_id:(Int64.of_int epoch_id) items
...
let next_root_from_hashes_i64 ~prev ~epoch_id ~start_txid hashes =
  next_root_i64
    ~prev
    ~epoch_id
    (items_from_hashes ~start_txid hashes)
