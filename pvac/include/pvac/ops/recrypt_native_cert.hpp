...
inline size_t native_reset_layer_trace_base_len(size_t) {
    return 14 + 8 * 5;
}
...
inline NativeResetLayerTraceDecision decide_native_reset_layer_trace(const NativeResetLayerTraceProof& proof, const std::array<uint8_t, 32>& old_layer_digest, const std::array<uint8_t, 32>& new_layer_digest) {
    ...
    decision.transcript =
        (proof.kind == NativeResetLayerTraceKind::FIELD_NATIVE || proof.kind == NativeResetLayerTraceKind::BRANCH_SHA256) &&
    ...
    decision.sha =
        decision.response &&
        (
            (proof.kind == NativeResetLayerTraceKind::FIELD_NATIVE && proof.field_transition && field_decision.admitted) ||
            (proof.kind == NativeResetLayerTraceKind::BRANCH_SHA256 && native_reset_layer_trace_sha_stack_ok(proof))
        );
    decision.secrecy =
        decision.response &&
        proof.hides_witness &&
...
inline size_t native_reset_challenge_branch(size_t branches, const std::array<uint8_t, 32>& challenge, size_t round, size_t slot, const char* domain) {
    ...
    uint64_t limit = std::numeric_limits<uint64_t>::max() - (std::numeric_limits<uint64_t>::max() % branch_count);
